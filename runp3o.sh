...
CONFIG_NAME=1B accelerate launch --num_processes 1 --config_file ../../configs/accelerate/zero2-bf16.yaml p3o_hh.py

# Exit if script execution fails
if [ $? -ne 0 ]; then
    echo "Failed to execute ppo_hh.py with accelerate."
    exit 1
fi
