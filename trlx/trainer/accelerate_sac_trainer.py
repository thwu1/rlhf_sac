...
from trlx.models.modeling_sac import (
...
from trlx.pipeline.sac_pipeline import SACRolloutStorage
...
from trlx.models.modeling_sac import GPTModelBranch

logger = logging.get_logger(__name__)


@register_trainer
class AccelerateSACTrainer(AccelerateRLTrainer):
    """SAC Accelerate Trainer"""

    reward_fn: Callable[[List[str], List[str], List[str]], List[float]]
    tokenizer: AutoTokenizer

    def __init__(self, config: TRLConfig, **kwargs):
        """SAC Accelerate Trainer initialization
        ...
        self.store = SACRolloutStorage(self.tokenizer.pad_token_id, self.tokenizer.padding_side)
    ...
    def loss(self, batch: SACRLBatch):
        """Forward pass & loss
        ...
        rewards = batch.plain_rewards.to(self.accelerator.device)
        ...
        if self.config.model.model_arch_type == "seq2seq":
            ...
            logits = outputs.logits
            values_pred = outputs.value[:, :-1]
            values_tar = outputs.value_tar[:, :-1]
            logprobs = logprobs_of_labels(logits[:, :-1, :], tokens[:, 1:])

            probs = torch.nn.functional.softmax(logits, dim=-1)
            num_dist = probs.shape[0] * probs.shape[1]
            probs_reshaped = probs.view(num_dist, -1)

            outputs_adv = self.model.forward_adv(tokens, attention_mask, return_dict=True)
            adv_logits = outputs_adv.adv_logits
            adv_tar_logits = outputs_adv.adv_tar_logits
            ref_logits = outputs_adv.logits
            ref_logprobs = logprobs_of_labels(ref_logits[:, :-1, :], tokens[:, 1:])
            adv_logprobs = logprobs_of_labels(adv_logits[:, :-1, :], tokens[:, 1:])

            adv_tar_logprobs_new = torch.zeros_like(adv_logprobs)
            logprobs_new = torch.zeros_like(logprobs)

            for _ in range(self.config.method.num_new_actions):
                new_samples = torch.multinomial(probs_reshaped, num_samples=1).view(probs.shape[0], probs.shape[1])
                adv_tar_logprobs_new += (
                    logprobs_of_labels(adv_tar_logits[:, :-1, :], new_samples[:, :-1])
                    / self.config.method.num_new_actions
                )
                logprobs_new += (
                    logprobs_of_labels(logits[:, :-1, :], new_samples[:, :-1]) / self.config.method.num_new_actions
                )
            ...
            (
                ...
                logprobs[:, start:end],
                logprobs_new[:, start:end],
                ref_logprobs[:, start:end],
                values_pred[:, start:end],
                values_tar[:, start:end],
                adv_logprobs[:, start:end],
                adv_tar_logprobs_new[:, start:end],
        ...
        loss, stats = self.config.method.loss(
            ...
            adv_logprobs=adv_logprobs,
            adv_tar_logprobs_new=adv_tar_logprobs_new,
        )

        return loss, stats

    def loss_v(self, batch: SACRLBatch):
        """Forward pass & loss
        ...
        loss = self.config.method.loss_v(
            values=values_pred,
            old_values=old_values,
            returns=returns,
            mask=mask,
    ...
    def loss_old(self, batch: SACRLBatch):
        """Forward pass & loss
        ...
        response_length = old_rewards.shape[1]
        """
        query_tensors torch.Size([8, 450])
        response_tensors torch.Size([8, 129])
        old_logprobs torch.Size([8, 128])
        old_values torch.Size([8, 128])
        old_rewards torch.Size([8, 128])
        advantages torch.Size([8, 128])
        returns torch.Size([8, 128])
        response_length 128
        """
        advantages, returns = self.config.method.get_advantages_and_returns(old_values, old_rewards, response_length)
        ...
        if self.config.model.model_arch_type == "seq2seq":
            ...
            logits = outputs.logits
            values_pred = outputs.value
            values_tar = outputs.value_tar
            values_pred = values_pred[:, :-1]
            values_tar = values_tar[:, :-1]
            logprobs = logprobs_of_labels(logits[:, :-1, :], tokens[:, 1:])
            ...
            """
            logits torch.Size([8, 579, 50304])
            values_pred torch.Size([8, 578])
            logprobs torch.Size([8, 578])
            attention_mask torch.Size([8, 579])
            """

            start = query_tensors.shape[1] - 1
            end = start + response_length
            logprobs, ref_logprobs, values_pred, values_tar, adv_logprobs, mask = (
                logprobs[:, start:end],
                ref_logprobs[:, start:end],
                values_pred[:, start:end],
                values_tar[:, start:end],
            ...
            """
            after slicing
            values_pred torch.Size([8, 128])
            logprobs torch.Size([8, 128])
    ...
    def post_epoch_callback(self):
        ...
        self.store.dropout_history(max_history_size=self.config.train.max_history_size)
    ...
    def post_backward_callback(self):
        ...
        gamma = 0.01
        for target_param, copy_param in zip(self.model.v_tar_head.parameters(), self.model.v_head.parameters()):
            target_param.data.copy_((gamma * copy_param.data) + (1.0 - gamma) * target_param.data)

        policy_head = GPTModelBranch(
            self.model.base_model,
            num_layers_unfrozen=2,
        )

        # for target_param, copy_param in zip(self.model.adv_tar_head.parameters(), self.model.adv_head.parameters()):
        for target_param, copy_param in zip(self.model.adv_tar_head.parameters(), policy_head.parameters()):
            target_param.data.copy_((gamma * copy_param.data) + (1.0 - gamma) * target_param.data)
    ...
    def prepare_learning(self):
        ...
        self.n_updates_per_batch = self.config.method.sac_epochs
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        then appends SACElements to trainer's `store`.
        ...
        while len(sac_rl_elements) < num_rollouts:
            stats = {}
            ...
            if self.config.model.model_arch_type == "seq2seq":
                ...
                with torch.no_grad():
                    logits, *_, values, _, _, _ = self.model(
                        all_tokens,
                        attention_mask=attention_mask,
                    )
            ...
            for sample_idx in range(n_samples):
                ...
                plain_rewards = torch.zeros_like(rewards)
                plain_rewards[-2] += scores[sample_idx].cpu()

                sac_rl_elements.append(
                    SACRLElement(
            ...
            tbar.set_description(f"[rollout {len(sac_rl_elements)} / {num_rollouts}]")
        ...
        self.push_to_store(sac_rl_elements)
