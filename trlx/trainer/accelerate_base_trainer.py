...
class AccelerateRLTrainer(BaseRLTrainer):
    ...
    def __init__(self, config, **kwargs):  # noqa: C901
        ...
        # def get_relevent_from_config(config):
        #     max_len = config.train.max_history_size
        #     alpha = config.method.alpha
        #     gamma = config.method.gamma
        #     lam = config.method.lam
        #     num_new_actions = config.method.num_new_actions
        #     actor_coef = config.method.actor_coef
        #     adv_scale = config.method.adv_scale
        #     str = (
        #         f"maxlen{max_len}_alp{alpha}_gam{gamma}_lam{lam}_nna{num_new_actions}_acoef{actor_coef}_advs{adv_scale}"
        #     )
        #     return str

        # run_name = get_relevent_from_config(config)
    ...
    def setup_model(self):
        ...
        if self.config.model.model_arch_type == "seq2seq":
            ...
            model.base_model.gpt_neox.embed_in.requires_grad_(False)
