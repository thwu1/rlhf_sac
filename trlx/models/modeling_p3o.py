...
class P3OConfig(MethodConfig):
    ...
    def loss(
        self,
        logratio1: TensorType["batch_size"],
        logratio2: TensorType["batch_size"],
        rewards1: TensorType["batch_size"],
        rewards2: TensorType["batch_size"],
        old_logratios1: TensorType["batch_size"],
        old_logratios2: TensorType["batch_size"],
    ):
        ...
        q_diff = (rewards1 - rewards2 - 0.05 * (logratio1 - logratio2)).detach()
        ratio_old1 = torch.exp(logratio1 - old_logratios1.detach())
        ratio_old2 = torch.exp(logratio2 - old_logratios2.detach())
        ratio_mean = (ratio_old1.mean() + ratio_old2.mean()) / 2
        ratio_max = torch.max(ratio_old1.max(), ratio_old2.max())
        ratio_min = torch.min(ratio_old1.min(), ratio_old2.min())
        cliped_ratio_old1 = torch.clamp(ratio_old1, 1/self.cliprange_ratio, self.cliprange_ratio)
        cliped_ratio_old2 = torch.clamp(ratio_old2, 1/self.cliprange_ratio, self.cliprange_ratio)

        loss1 = -q_diff * cliped_ratio_old2.detach() * ratio_old1
        loss1_clip = -q_diff * cliped_ratio_old2.detach() * torch.clamp(ratio_old1, 1-self.cliprange, 1+self.cliprange)
        loss2 = q_diff * cliped_ratio_old1.detach() * ratio_old2
        loss2_clip = q_diff * cliped_ratio_old1.detach() * torch.clamp(ratio_old2, 1-self.cliprange, 1+self.cliprange)
        loss = (torch.max(loss1, loss1_clip) + torch.max(loss2, loss2_clip)) / 2

        loss = loss.mean()

        reward_diff_mean = torch.mean(rewards1 - rewards2).item()
        reward_diff_abs = torch.mean(torch.abs(rewards1 - rewards2)).item()
        reward_diff_std = torch.std(rewards1 - rewards2).item()
        logratio_chosen_mean = torch.mean(
            logratio1 * (rewards1 > rewards2).float() + logratio2 * (rewards2 > rewards1).float()
        ).item()
        ...
        return loss, {
            ...
            "reward_diff_abs": reward_diff_abs,
            "reward_diff_std": reward_diff_std,
            "logratio_chosen_mean": logratio_chosen_mean,
            "logratio_lose_mean": logratio_lose_mean,
            "logratio_gap_mean": logratio_gap_mean,
    ...
    def loss_old(
        ...
        """
        beta = 0.1
        loss = -F.logsigmoid(beta * (logratio1 - logratio2)) * F.sigmoid(rewards1 - rewards2) - F.logsigmoid(
            beta * (logratio2 - logratio1)
        ) * F.sigmoid(rewards2 - rewards1)
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def forward(
        ...
        if not return_dict:
            outputs = (outputs.logits,) + outputs[1:] + (None,)
            return outputs

        return CausalLMOutputWithValue(**outputs, value=None)
