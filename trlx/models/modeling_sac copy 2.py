...
class SACConfig(MethodConfig):
    ...
    def get_adv_gae(self, values, rewards, ref_logprobs, old_logprobs, mask):
        ...
        if use_whiten:
            whiten_adv = mask * (adv - adv_mean) / (adv_std + 1e-8)
        return whiten_adv.detach(), {
    ...
    def loss(
        ...
        old_q = self.get_q(old_logprobs, old_values, mask)
        ...
        if train_adv:
            ...
            q = self.get_q(logprobs.detach(), values, mask)
            q_tar_new = self.get_q(logprobs_new.detach(), old_values, mask)

        q_gae_tar = self.get_tar_q_gae(logprobs_new, q_tar_new, rewards, ref_logprobs, old_logprobs, mask)

        q_clip = torch.clamp(q, old_q - 0.2, old_q + 0.2)
        td = q - q_gae_tar
        td_clip = q_clip - q_gae_tar
        q_loss1 = td.pow(2)
        q_loss2 = td_clip.pow(2)
        q_loss = 0.5 * torch.max(q_loss1, q_loss2).sum() / n
        q_clipfreq = torch.sum((q_loss1 < q_loss2).float() * mask).item() / n
        ...
        adv, adv_info = self.get_adv_gae(values, rewards, ref_logprobs, old_logprobs, mask)
        # actor_loss = ((log_ratio - self.adv_scale * adv) ** 2).sum() / n
        ratio = torch.exp((logprobs - old_logprobs.detach()) * mask)
        pg_loss1 = -adv * ratio
        ...
        loss = q_loss + self.actor_coef * pg_loss
        ...
        stats = dict(
            ...
            values=dict(
                ...
                q_clipfreq=q_clipfreq,
            ),
            returns=get_tensor_stats(returns, mask, n),
            policy=dict(approx_kl=approx_kl.item(), pg_clipfrac=pg_clipfrac.item()),
